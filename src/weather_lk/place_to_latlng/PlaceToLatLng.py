...
import json
from weather_utils import JSONFile, Log
...
class PlaceToLatLng:
    ...
    def get_latlng(place: str):
        ...
        if not PlaceToLatLng.GMAPS_API_KEY:
            raise ValueError("Set GMAPS_API_KEY for coordinate enrichment")
        client = googlemaps.Client(PlaceToLatLng.GMAPS_API_KEY)
        geocode_result = client.geocode(f"{place}, Sri Lanka")
    ...
    def get_place_to_latlng():
        return json.loads(files("weather_lk.stations").joinpath("data/coordinates.json").read_text(encoding="utf-8"))
