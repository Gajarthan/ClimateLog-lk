...
from weather_utils import File, Log, public_data_url
...
class SummaryReadMe:
    ...
    def lines_country(self):
        lines = []
        temp_image_path = public_data_url(
            "charts",
            "country_temperature.png",
        )
        rain_image_path = public_data_url(
            "charts",
    ...
    def get_lines_temperature(self, window):
        ...
        for place in DISPLAY_PLACES:
            ...
            if window:
                label += f"-{window}days"
            image_path_temp = public_data_url("charts", "temperature", f"{label}.png")
            lines.extend(
                [
                    f"## {place} 🌡️",
                    "",
    ...
    def get_lines_rainfall(self, window):
        ...
        for place in DISPLAY_PLACES:
            ...
            if window:
                label += f"-{window}days"
            image_path_rain = public_data_url("charts", "rainfall", f"{label}.png")
            lines.extend(
                [
                    f"## {place} ☔",
                    "",
    ...
    def lines_coverage(self):
        ...
        for window in SummaryCoverage.COVERAGE_WINDOW_LIST:
            image_path = public_data_url(f"coverage-{window}days.png")
            lines.extend(
                [
                    f"### Last {window:,} days",
                    "",
